...
<x-app-layout>
    ...
    <div class="bg-white rounded-[20px] shadow-[0_4px_20px_-10px_rgba(0,0,0,0.05)] border border-slate-100 p-8">
        ...
        <div class="flex flex-col md:flex-row justify-between items-center gap-6 mb-8 border-b border-slate-100 pb-6">
            <div>
                <h3 class="text-2xl font-serif font-bold text-luxury-charcoal">Guest Database</h3>
                <p class="text-sm text-slate-500 mt-1">Manage guest information and contact details.</p>
            </div>

            <a href="{{ route('admin.pelanggan.create') }}" class="h-12 px-6 bg-luxury-charcoal hover:bg-black text-luxury-ivory text-xs font-bold uppercase tracking-widest rounded-xl transition-colors shadow-md flex items-center justify-center whitespace-nowrap">
                + Register Guest
            </a>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50/50">
                        ...
                        <th class="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest border-b border-slate-100">Guest Name</th>
                        ...
                        <th class="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest border-b border-slate-100">Registration Date</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse ($pelanggans as $index => $pelanggan)
                        <tr class="hover:bg-slate-50/50 transition-colors group">
                            ...
                            <td class="px-6 py-4">
                                <span class="font-serif font-bold text-luxury-charcoal text-base">{{ $pelanggan->nama_pelanggan }}</span>
                            </td>
                            <td class="px-6 py-4">
                                <span class="text-sm font-medium text-slate-600">{{ $pelanggan->kontak_pelanggan ?? 'No Contact' }}</span>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end space-x-3 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <a href="{{ route('admin.pelanggan.edit', $pelanggan->id_pelanggan) }}" class="text-slate-400 hover:text-luxury-gold transition-colors" title="Edit">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                    </a>
                                    @if(Auth::user()->role === 'admin')
                                    <form action="{{ route('admin.pelanggan.destroy', $pelanggan->id_pelanggan) }}" method="POST" class="inline-block" onsubmit="return confirm('Remove this guest from registry?');">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-16 text-center">
                                ...
                                <p class="text-sm font-medium text-slate-400">No guests in the registry.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
